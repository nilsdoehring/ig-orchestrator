# IONOS — brand identity

Brand partition key: `0d7e8bcb-999b-4f96-8a31-3cfb42959e22`

Categories are the frozen vocabulary for this dimension. The number after each atom is its
rank; only the top 12 per category reach a prompt.

## brand_context

- (100) IONOS's mission is to be the European Cloud Platform for digital success by providing robust infrastructure, expert guidance, and scalable solutions.
- (90) IONOS proudly emphasizes its European origin and the associated virtues of a social market economy, sustainability, and strict data protection.
- (80) The IONOS Cloud Platform serves over 11 million customers worldwide, ranging from DIY users to enterprise-scale custom solutions.

## personality

- (90) The IONOS brand embodies the hero archetype, acting as a confident, sustainable European counterpoint to the major global tech giants.
- (80) The core personality traits of the IONOS brand are confident, dedicated, ambitious, bold, engaging, friendly, helpful, encouraging, and inspiring.
- (80) As a hero brand, IONOS communicates with bold confidence in its value, but is never overbearing, arrogant, or patronizing.
