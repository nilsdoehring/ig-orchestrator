...
subject: knowledge/rules/ux-writing.md
team: IONOS
...
### `copy_principle.readability_formatting`

**[MUST]** Break up texts into easy-to-read sections.
...
- **Source:** `section Content Principles, page 1, line 102`

### `highlight_badges.text_length`

**[MUST]** Reduce the badge text to a maximum of three words.

- **Why:** To ensure quick and easy comprehension as one cognitive chunk.
- **Slots:** badge
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Highlight Badges, page 35`

### `ux_writing.error_message_no_blame_or_panic`

**[MUST]** Versuche nicht, Humor einzubauen, Sorgen oder Panik zu schüren (z.B. "Falsches Passwort!!"), oder den Benutzer zu beschuldigen; formuliere stattdessen z.B. "Das hat leider nicht funktioniert. Versuchen Sie es bitte noch einmal."

- **Scope:** language de-DE (specificity 1)
- **Slots:** error_message
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `Style Guide DEU.md:298`

### `ux_writing.error_message_structure`

**[MUST]** Fehlermeldungen müssen dem Benutzer mitteilen, was passiert ist, und wie er das Problem beheben kann.

- **Exhaustive:** this list is complete. Do not extend it.
- **Why:** Gute Fehlermeldungen erfüllen diese zwei Aufgaben.
- **Slots:** error_message
- **Channels:** web
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Fehlermeldungen schreiben, page 10`

### `ux_writing.error_message_tone`

**[MUST]** Versuchen Sie nicht, Humor in Fehlermeldungen einzubauen, Sorgen oder Panik zu schüren oder den Benutzer zu beschuldigen.
...
### `ux_writing.error_message_two_jobs`

**[MUST]** Gute Fehlermeldungen erfüllen zwei Aufgaben: mitteilen, was passiert ist, und sagen, wie der Nutzer das Problem beheben kann.

- **Scope:** language de-DE (specificity 1)
- **Slots:** error_message
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `Style Guide DEU.md:290-294`

### `ux_writing.need_to_know`

**[MUST]** Halten Sie Ihr Publikum auf einer 'Das-musst-Du-wissen-Basis' und überschütten Sie es nicht mit unnötigen Informationen.

- **Why:** Der Schlüssel zu guten UX-Texten ist die Bereitstellung der richtigen Inhalte für den richtigen Nutzer zur richtigen Zeit.
- **Slots:** error_message, info_text, tooltip
- **Channels:** web
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Über UX-Texte, page 10`

### `ux_writing.product_page_emphasis`
...
- **Why:** Quotation marks are rarely used on IONOS product pages.
- **Scope:** language it-IT (specificity 1)
- **Slots:** badge, body_text, cta_button, headline, subline
- **Channels:** web
- **Source:** `section Anführungszeichen, page 1`

### `ux_writing.tooltip_marketing_minimum`

**[MUST]** Beschränken Sie das Marketing in Tooltips auf ein Minimum und konzentrieren Sie sich darauf, dem Benutzer bei der Erledigung seiner Aufgabe zu helfen.

- **Why:** Gute Tooltips sind benutzerorientiert, nicht IONOS-orientiert.
- **Slots:** tooltip
- **Channels:** web
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Tooltips schreiben, page 10`

### `ux_writing.tooltip_user_oriented_not_marketing`

**[SHOULD]** Gute Tooltips sind benutzerorientiert, nicht IONOS-orientiert; Marketing-Sprache auf ein Minimum beschränken und dem Benutzer bei der Erledigung seiner Aufgabe helfen.

- **Scope:** language de-DE (specificity 1)
- **Slots:** tooltip
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `Style Guide DEU.md:302-304`

### `ux_writing.ui_alignment`

**[SHOULD]** Align copy with existing UI labels in the product interface (e.g., using 'AI-Inhalte' if it is used in the MyWebsite Now editor).

- **Caveat:** Do not change UI labels independently.
- **Why:** To maintain consistency with the existing product interface.
...
### `ux_writing.ui_label_consistency`

**[MUST]** Align with existing UI labels in the product interface; do not change them independently.

- **Exhaustive:** no — the source presents this as illustrative.
- **Why:** Orientierung an der bestehenden Produktoberfläche.
- **Scope:** language fr-FR (specificity 1)
- **Slots:** cta_button, nav_label, tooltip
- **Channels:** web
- **Human track only:** no prompt can satisfy this — it asks for an action outside text generation. Never inject it as an instruction.
- **Source:** `section KI / AI / IA – Terminologieregeln, page 3`

### `ux_writing.ui_labels`

**[SHOULD]** Align with the labels already used in the product UI of the target market rather than changing them independently (example: 'AI-Inhalte' in the German MyWebsite Now editor).

- **Exhaustive:** no — the source presents this as illustrative.
- **Slots:** badge, error_message, tooltip
- **Channels:** web
- **Human track only:** no prompt can satisfy this — it asks for an action outside text generation. Never inject it as an instruction.
- **Source:** `section KI / AI / IA – Terminologieregeln, page 1`

### `ux_writing.ui_labels_consistency`

**[MUST]** Orientieren Sie sich bei UI-Labels an der bestehenden Produktoberfläche (z. B. 'AI-Inhalte' im MyWebsite-Now-Editor) und ändern Sie diese nicht eigenständig.

- **Scope:** language de-DE · all languages (asserted by the source) · all markets (asserted by the source) (specificity 1)
- **Slots:** badge, nav_label
- **Channels:** web
- **Human track only:** no prompt can satisfy this — it asks for an action outside text generation. Never inject it as an instruction.
