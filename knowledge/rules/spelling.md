...
subject: knowledge/rules/spelling.md
team: IONOS
...
22 rules. Modality is the contract: **must** is never dropped, **should** and **may** yield to the budget in that order.
...
**[MUST]** Verwenden Sie für den Possessiv die Formulierung 'X von IONOS'.
...
### `spelling.ai_compounds_body`

**[MUST]** Use lowercase for the noun following 'AI' in body text compounds (e.g., 'AI features', 'AI tools', 'AI models', 'AI content', 'AI assistant', 'AI proposition').
...
### `spelling.american_english`

**[MUST]** Consistently use American spelling conventions, including '-ize/-ization' (not '-ise/-isation'), '-or' (e.g., color), '-er' (e.g., center), '-og' (e.g., catalog), and do not double the final 'l' in verbs when adding suffixes.
...
**[MUST]** Avoid using 'Anti-virus'; use 'antivirus' instead.
...
**[MUST]** Use British spelling conventions: -ise/-isation (not -ize/-ization), -our (colour), -re (centre), -ogue, and double 'l' for verbs. Use UK terms like 'package(s)' instead of 'plan(s)', 'online shop' instead of 'online store', 'data centres' instead of 'data centers', 'enquiry/enquiries' instead of 'inquiry/inquiries', and 'Search Engine Optimisation' instead of 'Search Engine Optimization'.
...
### `spelling.consistent_english_term_spelling`

**[MUST]** Wir pflegen bei englischen Begriffen eine einheitliche Schreibweise, z.B. "White Paper" statt "Whitepaper" und "Onlineshop" statt "Online-Shop".
...
- **Source:** `Style Guide DEU.md:449`

### `spelling.duden_recommendation`

**[SHOULD]** Entscheiden Sie sich stets für die vom Duden empfohlene Schreibweise eines Begriffs (z. B. 'potenziell' statt 'potentiell'), es sei denn, es liegt eine schwerwiegende Ausnahme wie ein Produktname vor.

- **Caveat:** Wenn keine schwerwiegende, gegenteilige Begründung vorliegt (Produktname o.ä.)
- **Why:** Der Duden ist das Standardwerk und Referenz für alle Fragestellungen rund um die deutsche Sprache.
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Source:** `section Best Practices, page 5`

### `spelling.hosting_synonym`

**[MAY]** You may use 'hospedaje' as a synonym for hosting.

- **Why:** Unlike ES which uses 'alojamiento'.
- **Scope:** language es-MX · all languages (asserted by the source) · all markets (asserted by the source) (specificity 1)
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Source:** `section Weitere markttypische Regeln`

### `spelling.hyphenated_terms`

**[MUST]** Always hyphenate 'drag-and-drop' and 'money-back guarantee'.
...
### `spelling.preferred_terms`

**[SHOULD]** Prefer the spellings 'email', 'startup', 'web hosting' (two words), and 'package(s)'.
...
### `spelling.reference_duden`

**[MUST]** Nutzen Sie den Duden als Rechtschreibreferenz (z. B. potenziell, Onlineshop, White Paper).
...
**[MUST]** Avoid using 'start-up'; use 'startup' instead.
...
### `spelling.tariff_plan`

**[MUST]** Use 'plan' or 'planes' instead of 'pack' or 'packs' for tariffs.

- **Why:** US influence, key difference from ES.
- **Scope:** language es-MX · all languages (asserted by the source) · all markets (asserted by the source) (specificity 1)
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Source:** `section Weitere markttypische Regeln`

### `spelling.webhosting_de`

**[MUST]** Verwenden Sie im deutschen Markt ausschließlich die Schreibweise 'Webhosting'.

- **Exhaustive:** this list is complete. Do not extend it.
- **Why:** Einheitliche Marken- und Produktbezeichnung.
- **Scope:** all languages (asserted by the source) · all markets (asserted by the source) (specificity 0)
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.

### `terminology.ai_spelled_out`

**[MUST]** Spell out Artificial Intelligence in Spanish as 'Inteligencia Artificial'.
...
### `terminology.cloud_not_nube`

**[MUST]** Use 'página web / sitio web', 'dominio', and 'cloud' (do NOT use 'nube' as a product term).
...
### `terminology.google_workspace_spelling`

**[MUST]** Der Produktname hat immer ein großes G und ein großes W: Google Workspace.

- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Source:** `section Terminologie/Styleguide, page 3`

### `terminology.ki_product_names`

**[MUST]** Schreiben Sie IONOS-KI-Produktnamen im Deutschen mit dem Präfix 'KI-' und Bindestrich (z. B. KI-Website-Generator), außer bei 'AI Model Hub'.
...
### `terminology.official_app_names`

**[MUST]** Beachten Sie immer die offiziellen Namen und die Schreibweise (Großbuchstaben, Leerzeichen...) der Anwendungen in Google Workspace.

- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Source:** `section Terminologie/Styleguide, page 3`

### `tone.avoid_technical_terms`

**[SHOULD]** Vermeiden Sie Fachbegriffe oder erklären Sie diese gegebenenfalls.
...
### `vocabulary.avoid_foreign_words`

**[SHOULD]** Vermeiden Sie die Nutzung von Fremdwörtern wann immer möglich.

- **Caveat:** wann immer möglich
- **Why:** Einfacher Satzbau stellt sicher, dass Leser auch bei Ablenkung die wichtigsten Aussagen erfassen können.
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
