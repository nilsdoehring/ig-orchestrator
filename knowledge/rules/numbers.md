...
subject: knowledge/rules/numbers.md
team: IONOS
...
39 rules. Modality is the contract: **must** is never dropped, **should** and **may** yield to the budget in that order.

### `numbers.always_digits`

**[MUST]** Schreiben Sie Zahlen im Fließtext immer als Ziffern (z. B. 1, 5, 12 – nie ausschreiben).

- **Caveat:** Ausnahme: 'Million' ausschreiben (außer in Grafiken).
...
### `numbers.always_write_as_digits`

**[MUST]** Schreibe Zahlen grundsätzlich als Ziffern, unabhängig davon ob in Headlines oder im Fließtext — das gilt auch für Zahlen bis einschließlich 12 (neuere Duden-Empfehlung).
...
- **Source:** `Style Guide DEU.md:487`

### `numbers.decimal_separator`

**[MUST]** Verwenden Sie ein Komma als Dezimaltrennzeichen (z. B. 1,50).
...
**[MUST]** Use a period as the decimal separator (e.g., 1.50).
...
**[MUST]** Use a period as a decimal separator.
...
**[MUST]** Use a comma (,) as a decimal separator (e.g., 1,50).
...
**[MUST]** Use a comma as the decimal separator (e.g., 1,50).
...
**[MUST]** Use a comma (,) as the decimal separator.
...
**[SHOULD]** Write 'million' out in lowercase or abbreviate as 'M' (e.g., 1 million / 1M).
...
**[SHOULD]** Write 'million' out or abbreviate as 'M'.
...
**[MUST]** Write millions as '1 millón'.
...
**[MUST]** Write million as '1 millón'.
...
**[MUST]** Write "million" in lowercase and separated by a space (e.g., 1 million).
...
### `numbers.million_format`

**[MUST]** Write 'million' as '1 million' or '1M'.
...
### `numbers.million_spelled_out_except_graphics`

**[MUST]** "Million" wird als Wort ausgeschrieben, außer in grafischen Darstellungen.

- **Scope:** language de-DE (specificity 1)
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `Style Guide DEU.md:493`

### `numbers.million_spelling`

**[MUST]** Schreiben Sie das Wort 'Million' im Fließtext aus.

- **Caveat:** Außer in Grafiken.
- **Scope:** language de-DE · all languages (asserted by the source) · all markets (asserted by the source) (specificity 1)
...
**[MUST]** Schreiben Sie das Wort 'Million' als Wort aus, außer in grafischen Darstellungen.

- **Caveat:** außer in grafischen Darstellungen
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Zahlen, page 18`

### `numbers.percent`

**[MUST]** Format percentages with the percent sign directly following the number (e.g., 99%).
...
**[MUST]** Write percent values with the % symbol directly following the number without spaces.
...
**[MUST]** Format percentages with a space before the percent sign (e.g., 99 %).
...
**[MUST]** Write percent values with a space before the percentage sign (e.g., '99 %').
...
**[MUST]** Place a space before the percent sign (e.g., 99 %).
...
### `numbers.percent_format`

**[MUST]** Formatieren Sie Prozentangaben mit einem Leerzeichen vor dem Prozentzeichen (z. B. 99 %).
...
### `numbers.percent_symbol`

**[SHOULD]** Nutzen Sie in der Regel das Prozent-Symbol (%) anstatt den Begriff auszuschreiben, mit einem Leerzeichen zwischen Zahl und Symbol (z. B. '99 %').

- **Caveat:** in der Regel
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Prozentangaben, page 16`

### `numbers.percent_symbol_with_space`

**[MUST]** Wir nutzen das Prozent-Symbol (%) statt den Begriff auszuschreiben; zwischen Zahl und Symbol steht ein Leerzeichen, z.B. "99 %".

- **Scope:** language de-DE (specificity 1)
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Overrides locale default:** `de-DE.percent.spacing`. A locale library would get this wrong here.
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `Style Guide DEU.md:471-473`

### `numbers.thousands_separator`

**[MUST]** Verwenden Sie einen Punkt als Tausendertrennzeichen (z. B. 1.000).
...
**[MUST]** Use a comma as the thousands separator (e.g., 1,000).
...
**[MUST]** Use a comma as a thousands separator.
...
**[MUST]** Use a space as a thousands separator only for numbers with 5 or more digits (e.g., 10 000).
...
**[MUST]** Use a space as the thousands separator (e.g., 1 000).
...
**[MUST]** Use a period (.) as the thousands separator.
...
**[MUST]** Verwenden Sie ab 1.000 einen Punkt als Tausendertrennzeichen (z. B. 1.000, 10.000, 100.000).
...
### `numbers.thousands_separator_period`

**[MUST]** Zahlen ab 1.000 benötigen einen Punkt als Tausendertrennzeichen: 1.000, 10.000, 100.000.

- **Scope:** language de-DE (specificity 1)
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `Style Guide DEU.md:491`

### `numbers.write_as_digits`

**[MUST]** Schreiben Sie Zahlen grundsätzlich als Ziffern, auch für Zahlen bis einschließlich 12.

- **Caveat:** In manchen Fällen kann es seltsam aussehen, die Zahl zu verwenden, weil es sich um einen Satz handelt, z. B. 'Machen Sie den richtigen ersten Eindruck'.
- **Why:** Dies entspricht einer neueren Duden-Empfehlung.
- **Slots:** unscoped — applies wherever this rule's scope matches.
- **Checked, not instructed:** verified against the output by `/check`. Costs no prompt budget.
- **Modality inferred:** the source had no modal verb. Treat with the corresponding caution when it conflicts with a stated one.
- **Source:** `section Zahlen, page 18`
