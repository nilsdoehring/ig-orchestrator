...
that produced it. If you can reach the Hub, call it (`skills/endpoints`) and use what it
hands back.
...
- **the vocabulary and the appetites are snapshots too.** They are carried here now
  (`knowledge/skills.md` for every skill's appetite and model, `knowledge/<partition>/entries.md`
  for the legal scope values on each axis), which they were not before — but they are as
  current as the last publish, not as current as the Hub.
